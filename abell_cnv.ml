...
let abellh = Hashtbl.create 255

let process ocnt lin' =
  ...
  incr ocnt;
  let abo = match rslt.(0) with Int n -> "ACO"^string_of_int n | _ -> "unknown" in
  let field' = List.flatten (List.map (function
    | Nul -> []
    | Str s -> [s]
